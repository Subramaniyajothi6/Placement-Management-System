...
const interviewController = {
    createInterview:async(req,res)=>{try {
        
        const interview = await Interview.create(req.body);
        res.status(200).json({success:true,data:interview,message:"Interview created successfully"});
    ...
    getAllInterviews:async (req,res)=>{try {
        const interviews = await Interview.find();
        res.status(200).json({success:true,count:interviews.length,data:interviews});
    } catch (error) {
        res.status(500).json({ success:false, message: error.message });
    }},
    getSingleInterview: async (req,res)=>{try {
        const interview = await Interview.findById(req.params.id,req.body,{new:true,runValidators:true});
        if(!interview){
    ...
    updateInterview:async (req,res)=>{try {
    ...
    deleteInterview:async(req,res)=>{try {
