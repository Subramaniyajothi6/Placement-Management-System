...
import { fetchInterviews, selectInterviewError, selectInterviewLoading, selectInterviewsByStudent } from '../../slices/interviewSlice';
   ...
   return (
    ...
    <div className="max-w-4xl mx-auto p-4">
      ...
      <table className="w-full border-collapse border border-gray-300">
        ...
        <tbody>
          {interviews.map(({ _id, job, interviewDate, location, interviewer, status }) => (
            <tr key={_id} className="hover:bg-gray-50">
              <td className="border border-gray-300 p-2">{job?.title || 'N/A'}</td>
              <td className="border border-gray-300 p-2">
                {new Date(interviewDate).toLocaleString()}
              </td>
              <td className="border border-gray-300 p-2">{location || 'N/A'}</td>
              <td className="border border-gray-300 p-2">{interviewer?.name || 'N/A'}</td>
              <td
